...
export function getFriends(prefix = ''): any {
    return async (dispatch) => {
        ...
        ws.sendMessage('ClientSnap', {x: 1, y: 1});
...
export function leaveParty(): any {
    return async (dispatch) => {
        const response = await transport.doGet(HttpConstants.LEAVE_PARTY);
        const json = await response.json();
        response.ok ? dispatch(party(false)) : dispatch(setError({ ...json }));
    }
}

export function setParty(data): any {
    console.log('SET PARTY: ', data);

    return (dispatch) => {
        dispatch(party(data.users.length ? data : false));
    }
}
